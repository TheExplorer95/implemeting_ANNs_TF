...
path_save_embeddings_train = os.path.join(project_path, 'results/embeddings/', modelname, 'train/')
path_save_embeddings_test = os.path.join(project_path, 'results/embeddings/', modelname, 'test/')
# location to load saved embeddings
path_load_embeddings_train = os.path.join(project_path, 'results/embeddings/', modelname, 'train/')
path_load_embeddings_test = os.path.join(project_path, 'results/embeddings/', modelname, 'test/')
# location to save figures and history of classifier
current_time = datetime.now()
datetime_str = current_time.strftime('%Y%m%d-%H%M%S')
path_save_classifier_plots = os.path.join(project_path, 'results/classifier/', modelname, datetime_str)
...
num_em_samples_per_data = 1 #10  # How often to sample from a single data to get different parts
...
if modelname == '1dconv_transformer/' or modelname == '1dconv_gru/':
    enc_model = '1d_conv'
    path_data_train = os.path.join(path_data_train, 'GTZAN')
    encoder_args = {"z_dim": z_dim,
                    "stride_sizes": [5, 4, 2, 2, 2],
                    "kernel_sizes": [10, 8, 4, 4, 4],
                    "n_filters": [512, 512, 512, 512, 512],
                    "activation": tf.nn.leaky_relu}
    data_generator_arguments = {"T": 27,  # timestep
                                "k": 3,  # timestep
                                "N": 8,  # number
                                "full_duration": 4,  # sec
                                "original_sr": 22050,  # Hz
                                "desired_sr": 4410,  # Hz
                                "data_path": path_data_train}  # str

# 2dconv encoder params (mel spectogram data)
elif modelname == '2dconv_gru/' or modelname == '2dconv_transformer/':
    enc_model = '2d_conv'
    path_data_train = os.path.join(path_data_train, 'GTZAN_mel_specs')
    encoder_args = {"z_dim": z_dim,
...
# attentionAR params
    ...
    ar_args = {'num_enc_layers': 2,  # num. transformer encoder blocks
